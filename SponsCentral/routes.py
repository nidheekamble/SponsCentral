...
def register():
    ...
    if form.validate_on_submit():
        ...
        if form.select.data == 'P':
            ...
            hashed_partypassword= bcrypt.generate_password_hash(form.password.data).decode('utf-8')
            partyuser = PartyUser(party_email=form.email.data , party_password=hashed_partypassword )
            ...
            hashed_sponsorpassword= bcrypt.generate_password_hash(form.password.data).decode('utf-8')
            sponsoruser = SponsorUser(sponsor_email=form.email.data, sponsor_password=hashed_sponsorpassword )
